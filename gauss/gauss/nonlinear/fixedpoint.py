...
class FixedPointMethod:
    ...
    def __init__(self, equation_str, initial_guess, g_equation_str=None,
        ...
        try:
            ...
            if g_equation_str is None:
                # Try to solve f(x) = 0 for x to get x = g(x)
                solutions = sp.solve(self.f, self.x)
                if not solutions:
                    raise ValueError("Could not automatically solve for x. Please provide g(x) explicitly.")
    ...
    def solve(self, show_steps=False):
        ...
        for i in range(self.max_iterations):
            try:
                ...
                x_new = self.evaluate_function(self.g, x_old)
                ...
                iteration_step = [
                    f"Iteration {i + 1}:",
                    f"  x_old = {x_old:.{self.precision}f}",
                    f"  x_new = g(x_old) = {x_new:.{self.precision}f}",
                    f"  f(x_new) = {f_val:.{self.precision}e}",
                    f"  |εₐ| = {rel_error:.6f}%",
                ...
                if show_steps:
                    ...
                    print(f"  x_old = {x_old:.10f}")
                    print(f"  x_new = g(x_old) = {x_new:.10f}")
                    print(f"  f(x_new) = {f_val:.10e}")
        ...
        if not self.converged and not self.error_message:
            ...
            self.relative_error = rel_error if 'rel_error' in locals() else None
            self.error_message = (
    ...
    def get_results(self):
        ...
        sig_figs = 0
        if self.converged and len(self.iteration_history) >= 2:
